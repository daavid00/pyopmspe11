...
The module reads benchmark geometry, builds Cartesian, tensor, or corner-point
grids, maps facies and reporting regions to cells, adds boundary pore volume,
and locates wells and pressure sensors before deck files are written.
"""
...
from pyopmspe11.utils.writefile import (
    write_corner_point_grid,
    write_keywords,
    write_opm_input_files,
    write_regular_spe11c_grid,
)


def generate_deck_files(cfg: Config) -> list[str]:
    """Generate grid, property, and OPM input files.

    The selected grid type determines how facies, reporting regions, boundary pore
    volume, wells, and sensors are mapped before files are written.
    ...
    """
    names = []
    polygons, facies, points = _read_facies_polygons(cfg)
    if cfg.grid == "corner-point":
        xc, zc, d_x, d_y, d_z, ycent, xmx, ymy = _build_corner_point_grid(cfg, points)
        names.append("GRID.INC")
        if cfg.spe11 == "spe11a":
            fipnum, fluxnum = _assign_corner_point_spe11a_properties(
                cfg, polygons, facies, xc, zc
            )
            names.extend(write_keywords(cfg, fipnum, fluxnum, xmx))
        else:
            fipnum, fluxnum, porv = _assign_corner_point_spe11bc_properties(
                cfg, polygons, facies, xc, zc, ymy, ycent, d_x, d_y, d_z
            )
            names.extend(write_keywords(cfg, fipnum, fluxnum, xmx, None, porv))
    else:
        if cfg.spe11 == "spe11a":
            fipnum, fluxnum, xmx, zmz = _assign_structured_spe11a_properties(
                cfg, polygons, facies
            )
            names.extend(write_keywords(cfg, fipnum, fluxnum, xmx, zmz))
        else:
            fipnum, fluxnum, porv, xmx, zmz = _assign_structured_spe11bc_properties(
                cfg, polygons, facies
            )
            if cfg.spe11 == "spe11c":
                names.append("GRID.INC")
            names.extend(write_keywords(cfg, fipnum, fluxnum, xmx, zmz, porv))
    names.extend(write_opm_input_files(cfg))
    return names


def _build_structured_grid_axes(cfg: Config) -> tuple[NDArray, NDArray, NDArray]:
...
def _assign_structured_spe11a_properties(
    ...
    lowpoly = _read_lower_region_polygon(cfg) if cfg.lower else Polygon()
    xmx, ymy, zmz = _build_structured_grid_axes(cfg)
    xcent, ycent, zcent = _calculate_cell_centers(xmx, ymy, zmz)
    ...
    with bar_ctx as bar_animation:
        for k in range(cfg.nxyz[2]):
            for i in range(cfg.nxyz[0]):
                ...
                gind = i + k * cfg.nxyz[0]
                n = 0
                order = _get_polygon_search_order(zcent[k], zmidbot, ztopbot)
                for ind in order:
                ...
                if cfg.lower and not lowpoly.contains(Point(xcent[i], zcent[k])):
                    n = 7
                fluxnum[gind] = n
                fipnum[gind] = _get_region_fipnum(
                    cfg,
                    xcent[i],
                    zcent[k],
                    i,
    ...
    _locate_structured_sensors(cfg, fipnum, xcent, ycent, zcent)
    _locate_structured_wells(cfg, xcent, ycent, zcent)
    return fipnum, fluxnum, xmx, zmz


def _assign_structured_spe11bc_properties(
    cfg: Config, polygons: list[Polygon], facies: list
) -> tuple[NDArray, NDArray, list, NDArray, NDArray]:
    """Assign geological and reporting properties on a structured SPE11B or SPE11C
    grid.
    ...
    if cfg.lower:
        lowpoly = _read_lower_region_polygon(cfg)
    xmx, ymy, zmz = _build_structured_grid_axes(cfg)
    xcent, ycent, zcent = _calculate_cell_centers(xmx, ymy, zmz)
    dx, dy, dz = _calculate_cell_sizes(xmx, ymy, zmz)
    ...
    spe11 = cfg.spe11
    z_c = _calculate_spe11c_elevation(cfg, ycent) if spe11 == "spe11c" else 0 * ycent
    show_progress = sys.stdout.isatty()
    if show_progress:
    ...
    with bar_ctx as bar_animation:
        for index_z in range(no_cells_z):
            ...
            order = _get_polygon_search_order(value_z, zmidbot, ztopbot)
            for index_x in range(no_cells_x):
                ...
                fluxnum[gind] = n
                fipnum[gind] = _get_region_fipnum(
                    cfg, value_x, value_z - z_c[0], index_x, n
                )
                pv = (
            ...
            for index_y in range(1, no_cells_y):
                ...
                for index_x in range(no_cells_x):
                    value_x = xcent[index_x]
                    fipnum[dst + index_x] = _get_region_fipnum(
                        cfg,
                        value_x,
                        value_z - z_c[index_y],
                        index_x,
    ...
    if spe11 == "spe11c":
        if not cfg.lower:
            _add_front_back_boundary_properties(
...
def _add_front_back_boundary_properties(
    cfg: Config,
    fipnum: NDArray,
    fluxnum: NDArray,
    porv: list,
    ...
    for k in range(no_cells_z):
        for i in range(no_cells_x - 2):
            ...
            _set_front_back_fipnum(cfg, fipnum, fluxnum, ind)


def _set_front_back_fipnum(
...
def _get_polygon_search_order(z: float, zmidbot: float, ztopbot: float) -> list[int]:
...
def _assign_corner_point_spe11a_properties(
    ...
    with bar_ctx as bar_animation:
        for i in range(nxz):
            ...
            z_val = zc[i]
            n = 0
            order = _get_polygon_search_order(z_val, zmidbot, ztopbot)
            ...
            for ind in order:
                if polygons[ind].contains(point):
                    ...
                    break
            fluxnum[gind] = n
            fipnum[gind] = _get_region_fipnum(cfg, x_val, z_val, i_x, n)
    dx = (xc - sensors[0][0]) ** 2
...
def _assign_corner_point_spe11bc_properties(
    cfg: Config,
    polygons: list[Polygon],
    facies: list,
    xc: NDArray,
    ...
    """Assign geological and reporting properties on a corner-point SPE11B or SPE11C
    grid.
        ...
        for i in range(nxz):
            ...
            for ind in order:
                if polygons[ind].contains(point):
                    ...
                    break
            fluxnum[gind] = n
            fipnum[gind] = _get_region_fipnum(cfg, x_val, z_val - z_c[0], i_x, n)
            ...
            if i_x == no_cells_x - 1:
                ...
                for j in range(1, no_cells_y):
                    ...
                    for i_i in range(no_cells_x):
                        fipnum[dst + i_i] = _get_region_fipnum(
                            cfg,
                            xtemp[i_i],
                            ztemp[i_i] - z_c[j],
                            i_i,
    ...
    _locate_corner_point_wells_and_sensors(
        cfg, fipnum, zc, ymy, pop1, pop2, well1, well2
    )
    return fipnum, fluxnum, porv


def _locate_corner_point_wells_and_sensors(
    cfg: Config,
    fipnum: NDArray,
    zc: NDArray,
    ymy: NDArray,
    ...
    if cfg.spe11 == "spe11c":
        ...
        z_y = _calculate_spe11c_elevation(cfg, ycent)
        for k in range(cfg.nxyz[2]):
...
def _get_region_fipnum(
    cfg: Config, x_c: float, z_c: float, idx: int, fluxnum: float
) -> int:
    """Return the reporting-region FIPNUM for one cell.

    The reporting region depends on the cell-center coordinates, facies, and
    whether the cell lies on a lateral boundary.
    ...
    -------
    int
        FIPNUM identifying a benchmark box, lateral boundary, facies-1 region,
    ...
    if (
        ...
        return _get_facies_region_fipnum(fluxnum, 6, 3)
    if (
        (z_val >= cfg.boxc[0][2])
        & (z_val <= cfg.boxc[1][2])
        & (x_c >= cfg.boxc[0][0])
        ...
        return _get_facies_region_fipnum(fluxnum, 12, 4)
    if (
        (z_val >= cfg.boxa[0][2])
        & (z_val <= cfg.boxa[1][2])
        & (x_c >= cfg.boxa[0][0])
        ...
        return _get_facies_region_fipnum(fluxnum, 5, 2)
    if cfg.spe11 != "spe11a" and idx in (0, cfg.nxyz[0] - 1):
        return _get_facies_region_fipnum(fluxnum, 10, 11)
    if fluxnum == 1:
        return 7
    return 1


def _get_facies_region_fipnum(fluxnum: float, numa: int, numb: int) -> int:
...
def _calculate_spe11c_elevation(cfg: Config, ycent: NDArray) -> NDArray:
...
def _read_facies_polygons(cfg: Config) -> tuple[list[Polygon], list, list[Point]]:
...
def _read_lower_region_polygon(cfg: Config) -> Polygon:
...
def _read_horizon_lines(cfg: Config, points: list[Point]):
...
def _build_corner_point_grid(
    ...
    Horizon lines are interpolated and vertically refined before COORD and ZCORN
    are written and cell centers and sizes are calculated.
    ...
    """
    lines = _read_horizon_lines(cfg, points)
    ...
    xcoord, zcoord, cfg.nxyz[0], cfg.nxyz[2] = _refine_vertical_grid(
    ...
    d_x = xmx[1:] - xmx[:-1]
    write_corner_point_grid(cfg, xcoord, ymy, zcoord)
    no_cells_x = cfg.nxyz[0]
    no_cells_z = cfg.nxyz[2]
